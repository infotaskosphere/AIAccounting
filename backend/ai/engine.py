...
ai/engine.py
============
Fully self-contained AI engine for Finix Accounting.
NO external AI API dependencies (no Claude, no Gemini, no ChatGPT).

Features:
  1. BankStatementParser  — parses SBI / HDFC / ICICI / Axis / Kotak /
                            Yes Bank / IDFC / Federal / PNB / BOI /
                            any CSV / Excel / text bank statement
  2. InvoiceParser        — extracts invoice fields from PDF / image
  3. TransactionClassifier — 300+ rule patterns for Indian businesses
  4. ConfidenceScorer     — assigns confidence based on pattern strength

All intelligence is built-in using pdfplumber, openpyxl, regex, and
rapidfuzz — all already in requirements-render.txt.
"""
...
class BankStatementParser:
    """
    Parses bank statements from any Indian bank without any AI API.

    Supported inputs:
      • PDF (text-based)  — SBI, HDFC, ICICI, Axis, Kotak, Yes Bank,
                            IDFC First, Federal, PNB, BOI, Canara, UCO
      • CSV               — any bank CSV export
      • Excel (.xlsx/.xls)
      • Plain text

    Returns list of ParsedTransaction.
    """
    ...
    # ── Amount cleaning regex ─────────────────────────────────────────────────
    AMOUNT_RE = re.compile(r"[\d,]+\.?\d*")

    # ── Known bank table column signatures ───────────────────────────────────
    # Maps bank name → (date_col, narration_col, debit_col, credit_col, balance_col)
    # All values are lowercase partial matches for flexible header detection
    BANK_SIGNATURES = {
        "sbi": {
            "date":      ["txn date", "date"],
    ...
    def _parse_pdf(self, content: bytes) -> list[ParsedTransaction]:
        if not PDF_AVAILABLE:
            raise RuntimeError(
                ...
                "Add 'pdfplumber' to requirements-render.txt."
            )
        transactions: list[ParsedTransaction] = []

        with pdfplumber.open(io.BytesIO(content)) as pdf:
            for page in pdf.pages:
                # Try extracting as table first (works for most bank PDFs)
    ...
    def _map_columns(self, headers: list[str], bank: str = "generic") -> dict:
        """Map column names to indices using bank signature."""
                ...
                if result[field_name] is None:
                    for kw in keywords:
                        if kw in h_clean:
                            result[field_name] = i
                            break
        # Fall back to generic if any field still None
    ...
    def _row_to_transaction(self, row: list, col: dict) -> Optional[ParsedTransaction]:
        """Convert a table row to ParsedTransaction using column mapping."""
        def get(idx):
        ...
        # Amount — try debit/credit columns first
        debit_str  = get(col["debit"])
        credit_str = get(col["credit"])
        ...
        if ext == "xls":
            try:
                ...
                for r in range(ws.nrows):
                    rows.append([str(ws.cell_value(r, c) or "") for c in range(ws.ncols)])
            except Exception:
                return []
        else:
    ...
    def _parse_text_lines(self, lines: list[str]) -> list[ParsedTransaction]:
        """
        Regex-based line parser for raw text bank statements.
        Handles SBI, HDFC, ICICI text output.
        """
        transactions = []

        # Pattern: date + narration + optional debit + optional credit + balance
        # Matches patterns like:
        #   04 Apr 2025  BY TRANSFER-UPI/...  1,770.00  28,672.17
        #   04/04/2025   NEFT TO XYZ        5,000.00        1,23,456.78
        PATTERN = re.compile(
            r"(\d{1,2}[\s/-](?:\w{3}|\d{2})[\s/-]\d{2,4})"  # date
            r"\s+(.+?)\s+"                                     # narration
            r"([\d,]+\.?\d*)\s*"                               # amount1
            r"(?:([\d,]+\.?\d*)\s*)?"                          # amount2 (optional)
            r"([\d,]+\.?\d*)\s*$",                             # balance
            re.MULTILINE
        )
        ...
        for line in lines:
            ...
            if amt1 > 0 and amt2 > 0:
                # Two amounts — first is debit, second is credit typically
                # Use narration keywords to determine
    ...
    def _parse_date(self, raw: str) -> Optional[str]:
        """Parse date string to YYYY-MM-DD. Returns None if unparseable."""
        if not raw:
            return None
        ...
        # Try numeric-only: 20250404 → 2025-04-04
    ...
    def _parse_amount(self, raw: str) -> float:
        """Parse amount string like '1,23,456.78' → 123456.78."""
        if not raw:
            return 0.0
    ...
    def _clean_narration(self, text: str) -> str:
        """Clean up narration text — remove extra whitespace and control chars."""
        text = re.sub(r"[\x00-\x1f\x7f]", " ", text)
        text = re.sub(r"\s+", " ", text).strip()
        # Remove common PDF artifacts
        text = re.sub(r"\bNaN\b", "", text)
        return text[:200]  # cap at 200 chars

    def _is_credit_narration(self, narration: str) -> bool:
        """Heuristic: is this likely a credit (money in)?"""
        n = narration.upper()
        credit_kw = [
            "BY TRANSFER", "BY CLEARING", "CREDIT", "RECEIVED", "RECEIPT",
            "REFUND", "REVERSAL", "RETURN", "DIVIDEND", "INTEREST CREDIT",
            "UPI/CR", "NEFT/CR", "IMPS/CR", "CR/", "/CR/",
            "BULK POSTING",  # SBI credit posting
        ]
        debit_kw = [
...
class TransactionClassifier:
    """
    Classify Indian bank transaction narrations into accounting ledger accounts.
    300+ patterns covering common Indian business transactions.
    No external AI API required.
    """

    # ── Master rule table ─────────────────────────────────────────────────────
    # Format: (keywords_list, account_name, confidence)
    ...
    RULES: list[tuple[list[str], str, float]] = [
        ...
        (["salary", "sal/", "salaries", "payroll", "wages", "pay slip",
          ...
          "empl pay", "/sal/"], "Salaries & Wages", 0.95),
        ...
        (["emi", "loan repayment", "loan emi", "mortgage", "home loan",
          ...
          "loan payment", "principal repayment"],
         "Loan Repayment", 0.94),
        ...
        (["repair", "maintenance", "amc", "service charge for machine",
          ...
          "generator", "ac service", "vehicle service"],
         "Repairs & Maintenance", 0.89),

        # ── NEFT/UPI/IMPS Transfers — classify by direction ──────────────────
        # These are handled last as they are generic
        ...
        # ── Cheque-related ────────────────────────────────────────────────────
        (["cheque deposit", "chq deposit", "by clearing",
          "cheque return", "out-chq return", "chq return"],
         "Miscellaneous Income", 0.70),

        (["cheque issued", "chq issued", "to clearing"],
         "Miscellaneous Expense", 0.70),
    ...
    VENDOR_MAP: dict[str, tuple[str, float]] = {
        ...
        # Known clients/income (SPECTEVO/PRODIGIST accounts)
        ...
        "new india fire":("Sales Revenue", 0.95),
    ...
    def classify(self, narration: str) -> tuple[str, float]:
        """
        Classify a narration into (account_name, confidence).
        Uses vendor map first, then rules, then fuzzy fallback.
        """
        n = narration.upper().strip()

        # 1. Vendor-specific exact mapping (highest priority)
        ...
        for keywords, account, confidence in self.RULES:
            for kw in keywords:
                if kw.upper() in n:
                    # Boost confidence slightly for very strong matches
                    boost = 0.03 if len(kw) > 6 else 0.0
                    return account, min(confidence + boost, 0.99)

        # 3. Fuzzy fallback for common patterns
        if FUZZY_AVAILABLE:
    ...
    def _fuzzy_classify(self, narration: str) -> Optional[tuple[str, float]]:
        """Fuzzy match against common narration patterns."""
        FUZZY_PATTERNS = [
    ...
    def classify_batch(
        ...
        """Classify a list of transactions and return ClassifiedTransaction list."""
        result = []
        for i, t in enumerate(transactions):
            account, confidence = self.classify(t.narration)
            # Fine-tune confidence based on amount (large amounts get lower confidence)
            if t.amount > 100000:
...
class InvoiceParser:
    """
    Extract invoice fields from PDF or image files.
    Uses pdfplumber for text extraction + regex for field detection.
    No external AI API required.
    """
    ...
    def _extract_from_text(self, text: str) -> ParsedInvoice:
        ...
        result.invoice_date = self._parse_date(find("date"))
        ...
        # If amount not found but total is, back-calculate
    ...
    def _extract_narration(self, text: str) -> str:
        """Try to extract a short description of goods/services."""
        patterns = [
...
# ═══════════════════════════════════════════════════════════════════════════════
# 4. CONVENIENCE FUNCTIONS (used by main.py endpoints)
# ═══════════════════════════════════════════════════════════════════════════════
...
def parse_and_classify_statement(
    content: bytes,
    filename: str,
) -> dict:
    """
    Full pipeline: parse bank statement → classify each transaction.
    ...
    if not raw_txns:
        raise ValueError(
            ...
            "Check that the file is a valid bank statement."
        )
...
def parse_invoice_file(content: bytes, filename: str) -> dict:
    """
    Parse an invoice PDF and return structured fields.
    """
    ext = filename.lower().rsplit(".", 1)[-1] if "." in filename else ""
    if ext == "pdf":
        inv = _invoice.parse_pdf(content)
    else:
        # For images — extract text via OCR if available, else return empty
        inv = ParsedInvoice()
