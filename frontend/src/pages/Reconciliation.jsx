...
// Smart bank reconciliation UI with AI match suggestions
import { useState, useEffect } from 'react'
import { reconcileApi } from '../api/client'
import {
  GitMerge, Check, X, RefreshCw, Loader, AlertTriangle,
  CheckCircle, Clock, Eye, Filter
} from 'lucide-react'
...
function TxnRow({ txn, onConfirm, onUnmatch }) {
  ...
  const conf = txn.ai_match_confidence ? Math.round(txn.ai_match_confidence * 100) : null
  ...
  return (
    ...
    >
      <td style={{ padding:'10px 14px', fontSize:12, color:'var(--text-3)', fontFamily:'var(--mono)' }}>
        {txn.txn_date}
      </td>
      ...
      <td style={{ padding:'10px 14px', textAlign:'right', fontFamily:'var(--mono)', fontSize:13, fontWeight:600,
        color: txn.txn_type === 'credit' ? 'var(--success)' : 'var(--danger)' }}>
        {txn.txn_type === 'credit' ? '+' : '-'}₹{Number(txn.amount).toLocaleString('en-IN')}
      </td>
      ...
      <td style={{ padding:'10px 14px', textAlign:'center' }}>
        {conf !== null ? (
          <div style={{
            ...
            color: conf >= 85 ? 'var(--success)' : conf >= 65 ? 'var(--warning)' : 'var(--danger)',
            fontWeight:600,
          }}>
      ...
      <td style={{ padding:'10px 14px' }}>
        <div style={{ display:'flex', gap:6, justifyContent:'flex-end' }}>
          {txn.status === 'review' && (
            <>
              <button onClick={() => onConfirm(txn)} style={{
                padding:'4px 10px', border:'1px solid var(--success-b)', borderRadius:5,
                background:'var(--success-l)', color:'var(--success)', cursor:'pointer', fontSize:11, fontWeight:600, display:'flex', alignItems:'center', gap:4,
              }}>
                <Check size={11} /> Confirm
              </button>
              <button onClick={() => onUnmatch(txn)} style={{
                padding:'4px 8px', border:'1px solid var(--border)', borderRadius:5,
                background:'var(--surface)', color:'var(--text-3)', cursor:'pointer', fontSize:11,
              }}>
                <X size={11} />
              </button>
            </>
...
export default function Reconciliation() {
  ...
  const load = async () => {
    ...
    try {
      const [sumRes, txnRes] = await Promise.all([
        reconcileApi.getSummary(),
        reconcileApi.getUnmatched(),
      ])
      setSummary(sumRes.data?.data || sumRes.data)
      setTxns(txnRes.data?.data || txnRes.data || [])
    } catch {
  ...
  const runAI = async () => {
    ...
    try {
      ...
      toast.success(`Auto-matched ${d.auto_matched} transactions!`)
      await load()
    } catch {
  ...
  const confirm = async (txn) => {
    try {
      await reconcileApi.confirmMatch(txn.id, txn.matched_voucher_id)
  ...
  const unmatch = async (txn) => {
    try {
      await reconcileApi.unmatch(txn.id)
    ...
    <div className="page-wrap page-enter">
      ...
      {summary && (
        <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', gap:12, marginBottom:20 }}>
          ...
          <SummaryCard label="Unmatched Value" value={`₹${Number(summary.unmatched_amount || 0).toLocaleString('en-IN', {maximumFractionDigits:0})}`}
            icon={Eye} color="var(--info)" />
        </div>
      )}
      ...
      <div style={{ marginBottom:16, padding:'10px 16px', background:'linear-gradient(135deg,var(--primary-l),#F5F3FF)', borderRadius:8, border:'1px solid var(--primary-m)', fontSize:12, color:'var(--text-2)', display:'flex', gap:12, alignItems:'center' }}>
        ...
        <span><strong>How it works:</strong> AI matches bank transactions to journal entries using amount, date (±3 days), and narration similarity. Matches above 90% confidence are auto-confirmed. Lower confidence matches need your review.</span>
      </div>
        ...
        <div style={{ padding:'10px 14px', borderBottom:'1px solid var(--border)', display:'flex', gap:8, alignItems:'center' }}>
          ...
          {['all','unmatched','review','matched'].map(f => (
            <button key={f} onClick={() => setFilter(f)} style={{
              ...
              background: filter===f ? 'var(--primary-l)' : 'transparent',
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={6} style={{ padding:40, textAlign:'center', color:'var(--text-3)' }}>
                  <Loader size={20} style={{ animation:'spin .8s linear infinite' }} />
                </td></tr>
              ) : filtered.length === 0 ? (
                <tr><td colSpan={6} style={{ padding:40, textAlign:'center', color:'var(--text-3)', fontSize:13 }}>
                  {filter === 'all' ? '✅ No pending transactions to reconcile' : `No ${filter} transactions`}
                </td></tr>
              ) : (
                filtered.map(txn => (
                  <TxnRow key={txn.id} txn={txn} onConfirm={confirm} onUnmatch={unmatch} />
                ))
              )}
            </tbody>
          </table>
